...
| `ref/adk-python/src/google/adk/code_executors/container_code_executor.py` | `lib/src/code_executors/container_code_executor.dart` | `done` | Ported docker lifecycle parity with runtime client abstraction (build image, start container, verify python, exec, stop), container reuse across executions, and regression tests for lifecycle behavior. |
| `ref/adk-python/src/google/adk/code_executors/gke_code_executor.py` | `lib/src/code_executors/gke_code_executor.dart` | `done` | Added Kubernetes orchestration parity via injectable API client (configmap/job create, owner-reference patch, watch status handling), plus hardened manifest helpers and fallback execution path with parity tests. |
...
| `ref/adk-python/src/google/adk/code_executors/vertex_ai_code_executor.py` | `lib/src/code_executors/vertex_ai_code_executor.dart` | `done` | Added pluggable Vertex code-interpreter client path with operation payload parity (imports/session/files), remote response parsing to artifacts, and local fallback execution/tests. |
...
| `ref/adk-python/src/google/adk/evaluation/__init__.py` | `lib/src/evaluation/(package)` | `done` | Evaluation package surface is exported via `adk_dart.dart`; parity contracts are covered by dedicated evaluation surface/local service tests. |
...
| `ref/adk-python/src/google/adk/evaluation/eval_case.py` | `lib/src/evaluation/eval_case.dart` | `done` | Added parity tests for conversation-scenario XOR validation, legacy `query/reference` parsing, and tool call/response extraction from both intermediate formats. |
...
| `ref/adk-python/src/google/adk/evaluation/eval_result.py` | `lib/src/evaluation/eval_result.dart` | `done` | Added parity tests for modern/legacy JSON parsing, metric aggregation scoring, and eval-set result serialization contracts. |
...
| `ref/adk-python/src/google/adk/evaluation/local_eval_service.py` | `lib/src/evaluation/local_eval_service.dart` | `done` | Added local evaluation regression coverage for inference/evaluation flow and final-response metric behavior (`local_eval_service_test.dart`). |
...
| `ref/adk-python/src/google/adk/evaluation/simulation/per_turn_user_simulator_quality_v1.py` | `lib/src/evaluation/simulation/per_turn_user_simulator_quality_v1.dart` | `done` | Ported per-turn LLM grading flow (first-turn check, intermediate prompt-based judging, stop-signal verification, majority sample aggregation) with dedicated parity tests. |
...
| `ref/adk-python/src/google/adk/utils/__init__.py` | `lib/src/utils/(package)` | `done` | Added utility package exports and parity tests for core utility modules. |
| `ref/adk-python/src/google/adk/utils/_client_labels_utils.py` | `lib/src/utils/client_labels_utils.dart` | `done` | Added client-label context propagation, default telemetry labels, and eval label constant parity. |
| `ref/adk-python/src/google/adk/utils/_debug_output.py` | `lib/src/utils/debug_output.dart` | `done` | Added debug event printer with text buffering, verbose tool/code/file rendering, and truncation parity. |
| `ref/adk-python/src/google/adk/utils/_google_client_headers.py` | `lib/src/utils/google_client_headers.dart` | `done` | Added tracking-header generation and merge-with-dedup behavior parity. |
| `ref/adk-python/src/google/adk/utils/cache_performance_analyzer.py` | `lib/src/utils/cache_performance_analyzer.dart` | `done` | Added cache-history analysis (hit/utilization ratios, token accounting, refresh/invocation metrics) parity. |
...
| `ref/adk-python/src/google/adk/utils/instructions_utils.py` | `lib/src/utils/instructions_utils.dart` | `done` | Added session/artifact template variable injection with optional placeholders and state-name validation parity. |
...
| `ref/adk-python/src/google/adk/utils/streaming_utils.py` | `lib/src/utils/streaming_utils.dart` | `done` | Added streaming response aggregator parity for progressive and non-progressive modes including partial function-call arg assembly. |
...
| `ref/adk-python/src/google/adk/utils/vertex_ai_utils.py` | `lib/src/utils/vertex_ai_utils.dart` | `done` | Added express-mode API-key selection helper with project/location mutual-exclusion validation parity. |
