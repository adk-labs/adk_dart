...
void main() {
  group('RemoteA2aAgent parity', () {
    ...
    test(
      'mock input-required function response resumes A2A peer as text',
      () async {
        final _RecordingClient client = _RecordingClient(
          () => Stream<Object>.value(
            A2aMessage(
        ...
        final InvocationContext context = _context(
          ...
          events: <Event>[
            Event(
              ...
              author: 'remote_agent',
              customMetadata: <String, dynamic>{
                'a2a:task_id': 'task-mock',
                'a2a:context_id': 'ctx-mock',
              },
              longRunningToolIds: <String>{'mock-call-1'},
              content: Content(
                role: 'model',
                parts: <Part>[
                  Part.fromFunctionCall(
                    ...
                    id: 'mock-call-1',
                    args: <String, Object?>{
                      'input_required': 'Please choose a city.',
                    },
                  ),
                ],
              ),
              ...
              content: Content(
                ...
                parts: <Part>[
                  Part.fromFunctionResponse(
                    name: mockFunctionCallForRequiredUserInput,
                    id: 'mock-call-1',
                    response: <String, Object?>{'result': 'Seoul'},
                  ),
                ],
              ),
            ),
        ...
        expect(client.requests, hasLength(1));
        final A2aMessage request = client.requests.single;
        expect(request.taskId, 'task-mock');
        expect(request.contextId, 'ctx-mock');
        expect(request.parts.single.textPart?.text, 'Seoul');
